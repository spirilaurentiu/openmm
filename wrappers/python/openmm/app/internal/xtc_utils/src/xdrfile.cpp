...
const char* exdr_message[exdrNR] = {"OK",
                                    "Header",
                                    "String",
                                    "Double",
                                    "Integer",
...
static void
encodeints(int buf[], int num_of_ints, int num_of_bits, unsigned int sizes[], unsigned int nums[]) {
    int i;
...
void decodeints(int buf[], int num_of_ints, int num_of_bits, unsigned int sizes[], int nums[]) {
    int bytes[32];
...
int xdrfile_compress_coord_float(float* ptr, int size, float precision, XDRFILE* xfp) {
    ...
    while (i < size) {
        ...
        if (i + 1 < size) {
            if (abs(thiscoord[0] - thiscoord[3]) < smallnum && abs(thiscoord[1] - thiscoord[4]) < smallnum
...
void F77_FUNC(xdropen, XDROPEN)(int* fid, char* filename, char* mode, int fn_len, int mode_len) {
    char cfilename[512];
    char cmode[5];
    int i;
...
void F77_FUNC(xdrccs, XDRCCS)(int* fid, float* data, int* ncoord, float* precision, int* ret) {
    *ret = xdrfile_compress_coord_float(data, *ncoord, *precision, f77xdr[*fid]);
}


/* Read single-precision compressed 3d coordinates */
void F77_FUNC(xdrdcs, XDRDCS)(int* fid, float* data, int* ncoord, float* precision, int* ret) {
...
static int xdr_string(XDR* xdrs, char** cpp, unsigned int maxsize) {
    ...
    switch (xdrs->x_op) {
        case XDR_DECODE:
            ...
            if (sp == NULL) {
                () fputs("xdr_string: out of memory\n", stderr);
                return 0;
            }
            sp[size] = 0;
            /* fall into ... */
...
#    else
    ...
    if (ix == 0xdd || ix == 0x3f) {
        ...
        fprintf(stderr,
                "Cannot detect floating-point word order.\n"

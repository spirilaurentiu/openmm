...
static inline Dst ptr_cast_impl(Src p) noexcept {
    return (Dst)p;
}
} // namespace Support
...
namespace Support {
ASMJIT_FORCE_INLINE void* operatorNew(size_t n) noexcept {
    return malloc(n);
}
ASMJIT_FORCE_INLINE void operatorDelete(void* p) noexcept {
    if (p) {
        free(p);
    }
}
...
#    define ASMJIT_BASE_CLASS(TYPE)                                        \
        ...
        ASMJIT_FORCE_INLINE void operator delete(void* p) noexcept {       \
            Support::operatorDelete(p);                                    \
                                                                           ...
                                                                           \
        ASMJIT_FORCE_INLINE void* operator new(size_t, void* p) noexcept { \
            return p;                                                      \
        }                                                                  \
...
//!
//! Which yields 30 on 32-bit arch and 61 on 64-bit arch. The final value was adjusted by +1 for safety
//! reasons.
...
static inline Func ptr_as_func(void* func) noexcept {
    return Support::ptr_cast_impl<Func, void*>(func);
}

template <typename Func>
static inline void* func_as_ptr(Func func) noexcept {
...
enum ErrorCode : uint32_t {
    ...
    //! Invalid state.
    //!
    //! If this error is returned it means that either you are doing something wrong or AsmJit caught itself
    //! by doing something wrong. This error should never be ignored.
    kErrorInvalidState,
    ...
    kErrorInvalidLabel,
    //! Label index overflow - a single \ref BaseAssembler instance can hold almost 2^32 (4 billion) labels.
    ...
    //! Parent id passed to \ref CodeHolder::newNamedLabelEntry() was either invalid or parent is not
    //! supported by the requested `LabelType`.
    ...
    //! Instruction requires the use of consecutive registers, but registers in operands weren't (AVX512,
    //! ASIMD load/store, etc...).
    kErrorNotConsecutiveRegs,
    //! Failed to allocate consecutive registers - allocable registers either too restricted or a bug in RW
    //! info.
...
//!
//! Provided for debugging purposes. Putting a breakpoint inside `errored` can help with tracing the origin of
//! any error reported / returned by AsmJit.
static constexpr Error errored(Error err) noexcept {
    return err;
}
...
//! always returns errors on failures, assertions are a last resort and usually mean unrecoverable state due
//! to out of range array access or totally invalid arguments like nullptr where a valid pointer should be
//! provided, etc...
...
//!
//! Propagates a possible `Error` produced by `...` to the caller by returning the error immediately. Used by
//! AsmJit internally, but kept public for users that want to use the same technique to propagate errors to
//! the caller.

...
//!
//! It's used to make sure that when user links to an incompatible version of AsmJit, it won't link. It has
//! also some additional properties as well. When `ASMJIT_ABI_NAMESPACE` is defined by the user it would
//! override the AsmJit default, which makes it possible to use use multiple AsmJit libraries within a single
//! project, totally controlled by the users. This is useful especially in cases in which some of such library
...
// NOTE: Doxygen cannot document macros that are not defined, that's why we have to define them and then
// undefine them immediately, so it won't use the macros with its own preprocessor.
#ifdef _DOXYGEN
namespace asmjit {
...
//! \def ASMJIT_VIRTAPI
//!
//! This is basically a workaround. When using MSVC and marking class as DLL export everything gets exported,
//! which is unwanted in most projects. MSVC automatically exports typeinfo and vtable if at least one symbol
//! of the class is exported. However, GCC has some strange behavior that even if one or more symbol is
//! exported it doesn't export typeinfo unless the class itself is decorated with "visibility(default)" (i.e.
//! ASMJIT_API).
...
#else
#    define ASMJIT_FALLTHROUGH (() 0) /* fallthrough */
#endif
...
// Utilities.
#define ASMJIT_OFFSET_OF(STRUCT, MEMBER) \
    ((int)(intptr_t)((const char*)&((const STRUCT*)0x100)->MEMBER) - 0x100)
...
#    define ASMJIT_BEGIN_NAMESPACE                                                                          \
        ...
        _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wconstant-logical-operand\"") \
...
#    define ASMJIT_BEGIN_NAMESPACE                                                                      \
        namespace asmjit {                                                                              \
        inline namespace ASMJIT_ABI_NAMESPACE {                                                         \
        __pragma(warning(push)) __pragma(warning(disable : 4127)) /* conditional expression is const */ \
...
#    define ASMJIT_DEFINE_ENUM_FLAGS(T)                                                        \
        ...
        static ASMJIT_FORCE_INLINE T& operator|=(T& a, T b) noexcept {                         \
            a = T((std::underlying_type<T>::type)(a) | (std::underlying_type<T>::type)(b));    \
            return a;                                                                          \
        }                                                                                      \
        static ASMJIT_FORCE_INLINE T& operator&=(T& a, T b) noexcept {                         \
            a = T((std::underlying_type<T>::type)(a) & (std::underlying_type<T>::type)(b));    \

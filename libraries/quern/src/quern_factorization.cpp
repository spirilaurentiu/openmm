...
struct SparseEntry {
    ...
    SparseEntry(int index_, double value_)
        : index(index_)
        , value(value_) {
    }
};
...
static bool copy_row(int nnz, const int* index, const double* value, SparseVector& x) {
    for (int i = nnz - 1; i >= 0; --i) {
        if (value[i]) {
            if (!x.push_front(SparseEntry(index[i], value[i]))) {
                return false;
            }
        }
    }
...
static double encode(double c, double s) {
    if (std::fabs(s) < c) {
...
static void decode(double tau, double& c, double& s) {
    if (std::fabs(tau) < 1) {
        s = tau;
        c = std::sqrt(1 - s * s);
    } else {
...
static bool apply_givens(SparseVector& x, SparseVector& y, int diagonal, double& c, double& s) {
    ...
    while (p.still_going() && q.still_going()) {
        if (p->index == q->index) {
            double xnew = c * p->value - s * q->value, ynew = s * p->value + c * q->value;
            if (xnew) {
                p->value = xnew;
                ++p;
            } else {
            ...
            int k = p->index;
            double xnew = c * p->value, ynew = s * p->value;
            p->value = xnew;
            ++p;
            if (!y.insert(q, SparseEntry(k, ynew))) {
                return false;
            }
            ++q;
        } else {
            int k = q->index;
            double xnew = -s * q->value, ynew = c * q->value;
            if (!x.insert(p, SparseEntry(k, xnew))) {
                return false;
            }
            ++p;
            q->value = ynew;
...
int QUERN_compute_qr_without_q(int m,
                               ...
                               double** ptr_R_value) {
    if (m <= 0 || n <= 0 || m < n || !A_row_start || !A_column_index || !A_value || !ptr_R_row_start
        || !ptr_R_column_index || !ptr_R_value) {
...
int QUERN_compute_qr(int m,
                     ...
                     int** ptr_R_column_index,
                     double** ptr_R_value) {
    if (m <= 0 || n <= 0 || m < n || !A_row_start || !A_column_index || !A_value || !ptr_Q_row_start
        || !ptr_Q_column_index || !ptr_Q_value || !ptr_R_row_start || !ptr_R_column_index || !ptr_R_value) {
        return QUERN_INPUT_ERROR;
    }
...
static bool
apply_givens(SparseVector& x, SparseVector& y, int diagonal, const double* drop, double& c, double& s) {
    assert(!x.empty() && x.front().index == diagonal && x.front().value);
    assert(!y.empty() && y.front().index == diagonal && y.front().value);
    // find the rotation we need
    double a = x.front().value, b = y.front().value;
...
int QUERN_compute_incomplete_qr_without_q(int m,
    ...
    determine_column_thresholds(m, n, A_row_start, A_column_index, A_value, drop_tolerance, drop);
    // set up lists for dynamically building R
    Quern::Pool<SparseEntry> pool;
    SparseVector* R = (SparseVector*)std::malloc(m * sizeof(SparseVector));
    if (!R) {
...
void QUERN_free_result(int* row_start, int* column_index, double* value) {
    std::free(row_start);

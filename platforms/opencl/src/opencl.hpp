    ...
    int main()
    {
        ...
        std::string kernel2{R"CLC(
            ...
            kernel void vectorAdd(global const Foo* aNum, global const int *inputA, global const int *inputB,
                                  global int *output, int val, write_only pipe int outPipe, queue_t
 childQueue)
            {
              output[get_global_id(0)] = inputA[get_global_id(0)] + inputB[get_global_id(0)] + val +
 *(aNum->bar); write_pipe(outPipe, &val); queue_t default_queue = get_default_queue(); ndrange_t ndrange =
 ndrange_1D(get_global_size(0)/2, get_global_size(0)/2);
...
#else
static inline cl_int errHandler(cl_int err, const char* errStr = NULL) {
    () errStr; // suppress unused variable warning
    return err;
}
#endif // CL_HPP_ENABLE_EXCEPTIONS
} // namespace detail
...
namespace detail {
static inline cl_int buildErrHandler(cl_int err, const char* errStr, const BuildLogType& buildLogs) {
    () buildLogs; // suppress unused variable warning
    () errStr;
    return err;
}
} // namespace detail
...
class Context : public detail::Wrapper<cl_context> {
    ...
    Context(const vector<Device>& devices,
        ...
        object_ =
            ::clCreateContext(properties, (cl_uint)numDevices, deviceIDs.data(), notifyFptr, data, &error);
...
#    endif
    ...
    cl_int getSupportedImageFormats(cl_mem_flags flags,
        ...
        if (numEntries > 0) {
            ...
            err = ::clGetSupportedImageFormats(object_,
                                               flags,
                                               type,
                                               numEntries,
...
class Event : public detail::Wrapper<cl_event> {
     ...
     */
    cl_int
    setCallback(cl_int type, void(CL_CALLBACK* pfn_notify)(cl_event, cl_int, void*), void* user_data = NULL) {
...
template <typename IteratorType>
cl_int
copy(const CommandQueue& queue, IteratorType startIterator, IteratorType endIterator, cl::Buffer& buffer);
template <typename IteratorType>
...
template <typename T>
inline cl_int enqueueMapSVM(T* ptr,
                            cl_bool blocking,
                            cl_map_flags flags,
                            size_type size,
 ...
 *
 * Note that while this behaves like an allocator for the purposes of constructing vectors and similar
 * objects, care must be taken when using with smart pointers. The allocator should not be used to construct a
 * unique_ptr if we are using coarse-grained SVM mode because the coarse-grained management behaviour would
 * behave incorrectly with respect to reference counting.
 *
...
#    endif // #if defined(CL_HPP_ENABLE_EXCEPTIONS)
     ...
     */
    inline bool operator==(const SVMAllocator& rhs) {
        return (context_ == rhs.context_);
    }

    inline bool operator!=(const SVMAllocator& a) {
        return !operator==(a);
    }
...
class Kernel : public detail::Wrapper<cl_kernel> {
    ...
    cl_int getSubGroupInfo(const cl::Device& dev,
                           cl_kernel_sub_group_info name,
                           const cl::NDRange& range,
                           size_type* param) const {
#    if CL_HPP_TARGET_OPENCL_VERSION >= 210

        return detail::errHandler(clGetKernelSubGroupInfo(object_,
                                                          dev(),
                                                          name,
                                                          range.size(),
                                                          range.get(),
...
#    else // #if CL_HPP_TARGET_OPENCL_VERSION >= 210
        ...
        return detail::errHandler(pfn_clGetKernelSubGroupInfoKHR(object_,
                                                                 dev(),
                                                                 name,
                                                                 range.size(),
                                                                 range.get(),
...
#    endif // #if !defined(CL_HPP_CL_1_2_DEFAULT_BUILD)
     ...
     * \param err if non-NULL will be set to CL_SUCCESS on successful operation or one of the following
     * errors: CL_INVALID_CONTEXT if context is not a valid context. CL_INVALID_VALUE if the length of devices
     * is zero; or if the length of binaries does not match the length of devices; or if any entry in binaries
     * is NULL or has length 0. CL_INVALID_DEVICE if OpenCL devices listed in devices are not in the list of
     * devices associated with context. CL_INVALID_BINARY if an invalid program binary was encountered for any
     * device. binaryStatus will return specific status for each device. CL_OUT_OF_HOST_MEMORY if there is a
    ...
    cl_int build(const vector<Device>& devices,
        ...
        cl_int buildError =
            ::clBuildProgram(object_, (cl_uint)devices.size(), deviceIDs.data(), options, notifyFptr, data);

        return detail::buildErrHandler(buildError, __BUILD_PROGRAM_ERR, getBuildInfo<CL_PROGRAM_BUILD_LOG>());
    }

    cl_int build(const Device& device,
                 const char* options = NULL,
                 void(CL_CALLBACK* notifyFptr)(cl_program, void*) = NULL,
                 void* data = NULL) const {
        cl_device_id deviceID = device();
...
class CommandQueue : public detail::Wrapper<cl_command_queue> {
...
#if CL_HPP_MINIMUM_OPENCL_VERSION < 200
            if (!useWithProperties) {
                object_ = ::clCreateCommandQueue(context(),
                                                 device(),
    ...
    cl_int enqueueWriteBuffer(const Buffer& buffer,
                              ...
                              Event* event = NULL) const {
        cl_event tmp;
        cl_int err = detail::errHandler(::clEnqueueWriteBuffer(object_,
                                                               buffer(),
                                                               blocking,
                                                               offset,
                                                               size,
     ...
     */
    template <typename T, class D>
    cl_int enqueueMapSVM(cl::pointer<T, D>& ptr,
                         cl_bool blocking,
                         cl_map_flags flags,
                         size_type size,
                         const vector<Event>* events = NULL,
    ...
    template <typename T, class D>
    cl_int
    enqueueUnmapSVM(cl::pointer<T, D>& ptr, const vector<Event>* events = NULL, Event* event = NULL) const {
        cl_event tmp;
        cl_int err = detail::errHandler(
            ::clEnqueueSVMUnmap(object_,
                                static_cast<void*>(ptr.get()),
    ...
    cl_int enqueueMigrateSVM(const cl::vector<T*>& svmRawPointers,
        ...
        cl_event tmp;
        cl_int err = detail::errHandler(
            ::clEnqueueSVMMigrateMem(object_,
                                     svmRawPointers.size(),
 ...
 */
    ...
    cl_int(CL_API_CALL* PFN_clEnqueueAcquireD3D10ObjectsKHR)(cl_command_queue command_queue,
                                                             cl_uint num_objects,
                                                             ...
                                                             cl_event* event);
    typedef CL_API_ENTRY
    cl_int(CL_API_CALL* PFN_clEnqueueReleaseD3D10ObjectsKHR)(cl_command_queue command_queue,
                                                             cl_uint num_objects,
                                                             const cl_mem* mem_objects,
                                                             cl_uint num_events_in_wait_list,
                                                             const cl_event* event_wait_list,
...
class DeviceCommandQueue : public detail::Wrapper<cl_command_queue> {
     ...
     */
    DeviceCommandQueue(const Context& context,
                       const Device& device,
     ...
     */
    static DeviceCommandQueue
    makeDefault(const Context& context, const Device& device, cl_uint queueSize, cl_int* err = nullptr) {
        cl_int error;
...
#    if CL_HPP_TARGET_OPENCL_VERSION >= 210
     ...
     */
    static DeviceCommandQueue updateDefault(const Context& context,
                                            const Device& device,
 ...
 */
template <typename T, class D>
inline cl_int enqueueMapSVM(cl::pointer<T, D> ptr,
                            cl_bool blocking,
                            cl_map_flags flags,
                            size_type size,
                            const vector<Event>* events = NULL,
...
template <typename T, class D>
inline cl_int
enqueueUnmapSVM(cl::pointer<T, D>& ptr, const vector<Event>* events = NULL, Event* event = NULL) {
    cl_int error;
    CommandQueue queue = CommandQueue::getDefault(&error);
    if (error != CL_SUCCESS) {
        return detail::errHandler(error, __ENQUEUE_UNMAP_MEM_OBJECT_ERR);
...
template <typename T, class Alloc>
inline cl_int
enqueueUnmapSVM(cl::vector<T, Alloc>& container, const vector<Event>* events = NULL, Event* event = NULL) {
    cl_int error;
    CommandQueue queue = CommandQueue::getDefault(&error);
    if (error != CL_SUCCESS) {
        return detail::errHandler(error, __ENQUEUE_UNMAP_MEM_OBJECT_ERR);
...
inline cl_int enqueueReadImage(const Image& image,
    ...
    return queue
        .enqueueReadImage(image, blocking, origin, region, row_pitch, slice_pitch, ptr, events, event);
}
...
inline cl_int enqueueWriteImage(const Image& image,
    ...
    return queue
        .enqueueWriteImage(image, blocking, origin, region, row_pitch, slice_pitch, ptr, events, event);
}
...
class KernelFunctor {
    ...
    Event operator()(const EnqueueArgs& args, Ts... ts) {
        ...
        args.queue_
            .enqueueNDRangeKernel(kernel_, args.offset_, args.global_, args.local_, &args.events_, &event);
    ...
    Event operator()(const EnqueueArgs& args, Ts... ts, cl_int& error) {
        ...
        error = args.queue_.enqueueNDRangeKernel(kernel_,
                                                 args.offset_,
                                                 args.global_,
                                                 args.local_,
                                                 &args.events_,

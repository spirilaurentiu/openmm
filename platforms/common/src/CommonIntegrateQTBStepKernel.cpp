...
void CommonIntegrateQTBStepKernel::initialize(const System& system, const QTBIntegrator& integrator) {
    ...
    int elementSize =
        (cc.getUseMixedPrecision() || cc.getUseDoublePrecision() ? sizeof(double) : sizeof(float));
    ...
    QTBIntegratorUtilities::findTypes(system,
                                      integrator,
                                      particleTypeVec,
                                      typeParticles,
                                      typeMassVec,
                                      typeAdaptationRateVec);
    ...
    ComputeProgram program =
        cc.compileProgram(cc.replaceStrings(CommonKernelSources::qtb, replacements), defines);
...
void CommonIntegrateQTBStepKernel::execute(ContextImpl& context, const QTBIntegrator& integrator) {
    ...
    if (stepIndex % segmentLength == 0) {
        if (temperature != prevTemp) {
            ...
            QTBIntegratorUtilities::calculateSpectrum(temperature,
                                                      friction,
                                                      dt,
                                                      numFreq,
                                                      thetaVec,
...
double CommonIntegrateQTBStepKernel::computeKineticEnergy(ContextImpl& context,
                                                          const QTBIntegrator& integrator) {
    return cc.getIntegrationUtilities().computeKineticEnergy(0.0);
}

void CommonIntegrateQTBStepKernel::getAdaptedFriction(ContextImpl& context,
                                                      int particle,
                                                      std::vector<double>& friction) const {
...
void CommonIntegrateQTBStepKernel::setAdaptedFriction(ContextImpl& context,
                                                      int particle,
    ...
    while (L > 1) {
        ...
        if (L % 7 == 0) {
            ...
            throw OpenMMException(
                "QTBIntegrator: Number of steps in a segment must be a multiple of powers of 2, 3, 5, and 7");
        }
        source << "{\n";
        L = L / radix;
        ...
        if (radix == 7) {
            ...
            source << "mixed2 b1 = "
                   << cc.doubleToString((cos(2 * M_PI / 7) + cos(4 * M_PI / 7) + cos(6 * M_PI / 7)) / 3 - 1)
                   << "*(d6+d4);\n";
            source << "mixed2 b2 = "
                   << cc.doubleToString((2 * cos(2 * M_PI / 7) - cos(4 * M_PI / 7) - cos(6 * M_PI / 7)) / 3)
                   << "*(d0-d4);\n";
            source << "mixed2 b3 = "
                   << cc.doubleToString((cos(2 * M_PI / 7) - 2 * cos(4 * M_PI / 7) + cos(6 * M_PI / 7)) / 3)
                   << "*(d4-d2);\n";
            source << "mixed2 b4 = "
                   << cc.doubleToString((cos(2 * M_PI / 7) + cos(4 * M_PI / 7) - 2 * cos(6 * M_PI / 7)) / 3)
                   << "*(d2-d0);\n";
            source << "mixed2 b5 = -(" << sign << ")*"
                   << cc.doubleToString((sin(2 * M_PI / 7) + sin(4 * M_PI / 7) - sin(6 * M_PI / 7)) / 3)
                   << "*(d7+d1);\n";
            source << "mixed2 b6 = -(" << sign << ")*"
                   << cc.doubleToString((2 * sin(2 * M_PI / 7) - sin(4 * M_PI / 7) + sin(6 * M_PI / 7)) / 3)
                   << "*(d1-d5);\n";
            source << "mixed2 b7 = -(" << sign << ")*"
                   << cc.doubleToString((sin(2 * M_PI / 7) - 2 * sin(4 * M_PI / 7) - sin(6 * M_PI / 7)) / 3)
                   << "*(d5-d3);\n";
            source << "mixed2 b8 = -(" << sign << ")*"
                   << cc.doubleToString((sin(2 * M_PI / 7) + sin(4 * M_PI / 7) + 2 * sin(6 * M_PI / 7)) / 3)
                   << "*(d3-d1);\n";
            ...
            source << "data" << output << "[base+(6*j+1)*" << m << "] = " << mult << "(w[j*" << size << "/"
                   << (7 * L) << "], t7-t10);\n";
            source << "data" << output << "[base+(6*j+2)*" << m << "] = " << mult << "(w[j*" << (2 * size)
                   << "/" << (7 * L) << "], t9-t12);\n";
            source << "data" << output << "[base+(6*j+3)*" << m << "] = " << mult << "(w[j*" << (3 * size)
                   << "/" << (7 * L) << "], t8+t11);\n";
            source << "data" << output << "[base+(6*j+4)*" << m << "] = " << mult << "(w[j*" << (4 * size)
                   << "/" << (7 * L) << "], t8-t11);\n";
            source << "data" << output << "[base+(6*j+5)*" << m << "] = " << mult << "(w[j*" << (5 * size)
                   << "/" << (7 * L) << "], t9+t12);\n";
            source << "data" << output << "[base+(6*j+6)*" << m << "] = " << mult << "(w[j*" << (6 * size)
                   << "/" << (7 * L) << "], t7+t10);\n";
        } else if (radix == 5) {
            source << "mixed2 c0 = data" << input << "[base];\n";
            source << "mixed2 c1 = data" << input << "[base+" << (L * m) << "];\n";
            ...
            source << "mixed2 d9 = " << sign << "*make_mixed2(d2.y+" << coeff << "*d3.y, -d2.x-" << coeff
                   << "*d3.x);\n";
            source << "mixed2 d10 = " << sign << "*make_mixed2(" << coeff << "*d2.y-d3.y, d3.x-" << coeff
                   << "*d2.x);\n";
            source << "data" << output << "[base+4*j*" << m << "] = c0+d4;\n";
            source << "data" << output << "[base+(4*j+1)*" << m << "] = " << mult << "(w[j*" << size << "/"
                   << (5 * L) << "], d7+d9);\n";
            source << "data" << output << "[base+(4*j+2)*" << m << "] = " << mult << "(w[j*" << (2 * size)
                   << "/" << (5 * L) << "], d8+d10);\n";
            source << "data" << output << "[base+(4*j+3)*" << m << "] = " << mult << "(w[j*" << (3 * size)
                   << "/" << (5 * L) << "], d8-d10);\n";
            source << "data" << output << "[base+(4*j+4)*" << m << "] = " << mult << "(w[j*" << (4 * size)
                   << "/" << (5 * L) << "], d7-d9);\n";
        } else if (radix == 4) {
            source << "mixed2 c0 = data" << input << "[base];\n";
            source << "mixed2 c1 = data" << input << "[base+" << (L * m) << "];\n";
            ...
            source << "data" << output << "[base+(3*j+1)*" << m << "] = " << mult << "(w[j*" << size << "/"
                   << (4 * L) << "], d1+d3);\n";
            source << "data" << output << "[base+(3*j+2)*" << m << "] = " << mult << "(w[j*" << (2 * size)
                   << "/" << (4 * L) << "], d0-d2);\n";
            source << "data" << output << "[base+(3*j+3)*" << m << "] = " << mult << "(w[j*" << (3 * size)
                   << "/" << (4 * L) << "], d1-d3);\n";
        } else if (radix == 3) {
            source << "mixed2 c0 = data" << input << "[base];\n";
            source << "mixed2 c1 = data" << input << "[base+" << (L * m) << "];\n";
            ...
            source << "mixed2 d2 = " << sign << "*" << cc.doubleToString(sin(M_PI / 3.0))
                   << "*make_mixed2(c1.y-c2.y, c2.x-c1.x);\n";
            source << "data" << output << "[base+2*j*" << m << "] = c0+d0;\n";
            source << "data" << output << "[base+(2*j+1)*" << m << "] = " << mult << "(w[j*" << size << "/"
                   << (3 * L) << "], d1+d2);\n";
            source << "data" << output << "[base+(2*j+2)*" << m << "] = " << mult << "(w[j*" << (2 * size)
                   << "/" << (3 * L) << "], d1-d2);\n";
        } else if (radix == 2) {
            source << "mixed2 c0 = data" << input << "[base];\n";
            source << "mixed2 c1 = data" << input << "[base+" << (L * m) << "];\n";
            source << "data" << output << "[base+j*" << m << "] = c0+c1;\n";
            source << "data" << output << "[base+(j+1)*" << m << "] = " << mult << "(w[j*" << size << "/"
                   << (2 * L) << "], c0-c1);\n";
        }
        source << "}\n";
        m = m * radix;
